...
class Bootstrap extends InitTemplate
{
    ...
    public function setup()
    {
        ...
        $this->say("tests/http created           <- http tests");
        $this->say("tests/http.suite.yml written <- http tests suite configuration");
        $this->createAcceptanceSuite();
        $this->say("tests/api created           <- api tests");
        $this->say("tests/api.suite.yml written <- api tests suite configuration");

        $this->say(" --- ");
        $this->say();
        $this->saySuccess('Codeception is installed for api, http, and unit testing');
        $this->say();

        $this->say("<bold>Next steps:</bold>");
        $this->say('1. Edit <bold>tests/api.suite.yml</bold> to set url of your application. Change PhpBrowser to WebDriver to enable browser testing');
        $this->say("2. Edit <bold>tests/http.suite.yml</bold> to enable a framework module. Remove this file if you don't use a framework");
        $this->say("3. Create your first api tests using <comment>codecept g:cest api First</comment>");
        $this->say("4. Write first test in <bold>tests/api/FirstCest.php</bold>");
    ...
    protected function createFunctionalSuite($actor = 'Http')
    {
        $suiteConfig = <<<EOF
# Codeception Test Suite Configuration
#
# Suite for http tests
...
EOF;
        $this->createSuite('http', $actor, $suiteConfig);
    }

    protected function createAcceptanceSuite($actor = 'Api')
    {
        $suiteConfig = <<<EOF
# Codeception Test Suite Configuration
#
...
EOF;
        $this->createSuite('api', $actor, $suiteConfig);
    }
